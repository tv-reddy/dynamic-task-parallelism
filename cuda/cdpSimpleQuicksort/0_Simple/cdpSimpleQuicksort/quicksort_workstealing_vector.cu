...
__device__ void append_task(thrust::device_vector<task> queue, task new_task) {
    queue.resize(queue.size() + 1);
    queue[queue.size() - 1] = new_task;
}
...
__global__ void cdp_simple_quicksort(unsigned int *data,
                                    int left, int right,
                                    int depth, thrust::device_vector<task> d_queue_1,
                                    thrust::device_vector<task> d_queue_2,
                                    unsigned int num_items)
{
    ...
    if (left < (rptr-data))
    {
        ...
        new_task->array = (int*)malloc(num_items * sizeof(int))
        new_task->left_ptr = (int*)malloc(sizeof(int));
        new_task->right_ptr = (int*)malloc(sizeof(int));
...
////////////////////////////////////////////////////////////////////////////////
void run_qsort(unsigned int *data, unsigned int nitems, thrust::device_vector<task> d_queue_1, thrust::device_vector<task> d_queue_2)
{
    ...
    cdp_simple_quicksort<<< 1, 1 >>>(data, left, right, 0, d_queue_1, d_queue_2, nitems);
...
int main(int argc, char **argv)
{
    ...
    thrust::host_vector<task> host_task_queue_2(0);
    thrust::device_vector<task> device_task_queue_1 = host_task_queue_1;
    thrust::device_vector<task> device_task_queue_2 = host_task_queue_2;



    // Execute
    std::cout << "Running quicksort on " << num_items << " elements" << std::endl;
    run_qsort(d_data, num_items, device_task_queue_1, device_task_queue_2);
